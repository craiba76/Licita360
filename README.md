...
<body>
  ...
  <main>
    <div class="tabs">
      ...
      <button class="tab-button" onclick="abrirAba(event, 'concorrencia')">📈 Concorrência</button>
      <button class="tab-button" onclick="abrirAba(event, 'historico')">🕓 Histórico</button>
    </div>
    ...
    <div id="concorrencia" class="tab-content">
      <p>Módulo de concorrência em desenvolvimento.</p>
    </div>

    <div id="historico" class="tab-content">
      <p>Histórico de pesquisas aparecerá aqui.</p>
    </div>
  </main>
      ...
      try {
        ...
        docs.forEach(item => {
          ...
          div.innerHTML = `
            ...
            <strong>Município:</strong> ${item.municipio}<br>
            <strong>Data:</strong> ${item.data_abertura}<br>
        ...
        results.innerHTML = "<p>Erro ao buscar dados no PNCP.</p>";
        console.error(error);
      }
    }

    async function buscarComprasGov() {
      const termo = document.getElementById("searchInputComprasgov").value;
      const results = document.getElementById("resultsComprasgov");
      results.innerHTML = "<p>Buscando no Compras.gov.br...</p>";

      try {
        const resposta = await fetch(`https://api.compras.dados.gov.br/licitacoes/v1/licitacoes.json?descricao_objeto=${termo}`);
        const data = await resposta.json();

        if (!data._embedded || data._embedded.licitacoes.length === 0) {
          results.innerHTML = "<p>Nenhum resultado encontrado no Compras.gov.br.</p>";
          return;
        }

        results.innerHTML = "";
        data._embedded.licitacoes.forEach(item => {
          const div = document.createElement("div");
          div.className = "result-item";
          div.innerHTML = `
            <strong>Órgão:</strong> ${item.orgao.nome}<br>
            <strong>Objeto:</strong> ${item.objeto}<br>
            <strong>Modalidade:</strong> ${item.modalidade}<br>
            <strong>Data:</strong> ${item.data_abertura}
          `;
          results.appendChild(div);
        });
      } catch (error) {
        results.innerHTML = "<p>Erro ao buscar dados no Compras.gov.br.</p>";
        console.error(error);
      }
    }
  </script>
